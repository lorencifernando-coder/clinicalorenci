...
com foco em medicina integrativa, burnout e saúde mental. Site estático (HTML/CSS/JS puro),
sem back-end e sem processo de build — basta subir os arquivos num servidor web.
...
1. Suba **todos** os arquivos/pastas deste repositório para a raiz (ou subpasta) do seu servidor,
   preservando a estrutura acima.
2. O servidor deve entregar `index.html` como página padrão.
3. Não há compilação nem instalação de dependências. É um site estático.

**Requer internet no navegador do visitante** para carregar:
- Google Fonts (Playfair Display, Inter);
- Tailwind CDN (usado pelo tema base);
- os vídeos `.dc.html` (carregam React/Babel via `support.js`).

Funciona em qualquer hospedagem estática (Hostinger, Vercel, Netlify, GitHub Pages,
Apache/Nginx, etc.).
